...
} from "@keystatic/core/renderer"; // Correct import for Document type
...
const getValidDocument = (
  doc: DocumentRendererProps["document"] | null | undefined,
...
export function ExperiencePreview({
  ...
  const resolvedItems = items.map((experience) => ({
    // Removed async from map callback
    ...experience,
    entry: {
      ...experience.entry,
            ...
            >
              ...
              <div className="flex flex-col gap-3 max-w-xl flex-1">
                ...
                <p className="text-muted-foreground text-sm">
                  ...
                  {formatDate(experience.entry.startDate)} -{" "}
                  {experience.entry.endDate
                    ? formatDate(experience.entry.endDate)
                    : "Present"}
                </p>
                ...
                <div className="prose prose-invert max-w-none text-muted-foreground leading-relaxed">
                  <DocumentRenderer
                    document={getValidDocument(experience.entry.description)}
                  />
                </div>
              </div>
            </div>
